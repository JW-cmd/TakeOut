<template>
  <div>
    <section class="profile">
      ...
      <!-- 点击登录按钮有判断，如果已经登陆了就不再跳转到login界面 -->
      <section class="profile-number">
        <a href="javascript:"
           class="profile-link"
           @click="user._id ? '' : goTo('/login')">
          <div class="profile_image"
               @click.stop="changeAvater()">
            <i class="iconfont icon-person"></i>
          </div>
          <div class="user-info">
            <p class="user-info-top"
               v-if="!user.phone">{{user.name || '登录/注册'}}</p>
            <p>
              <span class="user-icon">
                <i class="iconfont icon-shouji icon-mobile"
                   :style="{verticalAlign : user.phone?'middle':'text-top'}"></i>
              </span>
              <span class="icon-mobile-number"
                    :class="{'havePhone':user.phone}"
                    style="margin-left:10px;">{{user.phone || '暂无绑定手机号'}}</span>
            </p>
          </div>
          <span class="arrow">
